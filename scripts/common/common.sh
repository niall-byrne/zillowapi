...
security() {
  ...
  pushd "${PROJECTHOME}"  > /dev/null
    bandit -r zillowapi -c .bandit.rc
    safety check
  popd  > /dev/null
...
lint() {
  ...
  pushd "${PROJECTHOME}"  > /dev/null
    yapf -i --recursive --exclude '**/*_pb2.py' --style='{based_on_style: google, COLUMN_LIMIT: 79, INDENT_WIDTH: 2, ALIGN_CLOSING_BRACKET_WITH_VISUAL_INDENT: false, DEDENT_CLOSING_BRACKETS: false}' "zillowapi/"
    isort -y
  popd  > /dev/null
